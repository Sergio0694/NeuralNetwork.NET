...
namespace NeuralNetworkNET.SupervisedLearning.Misc
{
    ...
    internal struct TrainingBatch
    {
        ...
        public static IReadOnlyList<TrainingBatch> FromDataset([NotNull] double[,] x, [NotNull] double[,] y, int size)
        {
            // Local parameters
            int
            ...
            if (size == samples) return new[] { new TrainingBatch(x, y) }; // Fake batch with the whole dataset
